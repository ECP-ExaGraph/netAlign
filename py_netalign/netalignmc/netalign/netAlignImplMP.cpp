                    ...
                    double* wperm, netalign_parameters opts, double* objective)
{
    ...
    other_max_computer omax1(li.values(), ns, size, 1);
    other_max_computer omax2(lj.values(), nt, size, 1);
        ...
        else
        {
            ...
            if(damping_type==2) {
                ...
                for(int i=0;i<size;i++) {
                    ...
                    int t=ic[i+1]-1;
                    for(int j=s;j<t;j++)
                        stvc[j]=stvc[j]+(1.0-damping_mult)*(st1vc[j]+st1vc[perm[j]]-2*beta);
                }
                //for(int i=0;i<snz;i++)
            ...
            else
            {
                ...
                for(int i=0;i<size;i++) {
                    ...
                    int t=ic[i+1]-1;
                    for(int j=s;j<t;j++)
                        stvc[j]=damping_mult*stvc[j]+(1.0-damping_mult)*(st1vc[j]+st1vc[perm[j]]-2*beta);
                }
                //for(int i=0;i<snz;i++)
...
#include "netAlignImplMPTasked.cpp"
