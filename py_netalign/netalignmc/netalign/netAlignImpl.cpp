...
: problemname(NULL), alpha(1.0), beta(1.0),
gamma(0.99), maxiter(80), alg("mp"), dampingtype(2),
batchrounding(20), verbose(false), finalize(true), quiet(false),
...
void netAlign(int argc, float alpha)
{
    ...
    cout<<dummy[0]<<" "<<dummy[1]<<endl;
    opts.parse(argc,dummy);
    opts.alpha=alpha;
    //if (!opts.parse(argc, params)) {
    //    return;
    //}
//    cout<<"Args: "<<argc<<" "<<**params<<" "<<*params<<" "<<params[0]<<" "<<params[1]<<endl;    
    string basename = opts.problemname; // get the root name for the input matrices
    string Afilename = "graph-A.mtx";
    string Bfilename = "graph-B.mtx";
    string Lfilename = "graph-L.mtx";
    ...
    if (opts.finalize == true && opts.verbose) {
    ...
    opts.outFile="out.log";
    if(opts.outFile != NULL)
    {
    	ofstream myfile;
		myfile.open(opts.outFile, ofstream::out);
