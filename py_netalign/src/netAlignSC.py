...
def netAlignSC(f1,f2,fl,alpha=1,beta=1,ft=None,input_dir=None):
    ...
    command=['/home/khan242/netAlign/netalignmc/netalign/netAlign', 'graph']
    alpha='-a '+str(alpha)
    beta='-b '+str(beta)
    output='-o'+ 'sc.aln'
    ...
    command=['rm','sc.aln','graph-A.mtx','graph-B.mtx','graph-L.mtx']
    result = subprocess.run(command, stdout=subprocess.PIPE)  
...
if __name__ == '__main__':
    ...
    res_dir="/home/khan242/netAlign/results/synthetic_networks/edoardo/edoardo_scone/"
    ...
    df = pd.DataFrame(np.array(data),columns=['K=5%', 'K=10%', 'K=25%', 'K=50%', 'K=100%'], index=['a=1', 'a=0.75', 'a=0.5', 'a=0.25','a=0'])
    df.to_pickle("results_scone.pkl")
    #'''
